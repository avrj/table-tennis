...
const App = () => {
    ...
    const speakStatus = () => {
        ...
        if (pointsHome === pointsOpponent) {
            ...
            speak(`${homeName} siirtyi johtoon. tilanne ${pointsHome} - ${pointsOpponent}`);
        } else if (pointsOpponent > pointsHome && previousErotus === 0) {
            speak(`${opponentName} siirtyi johtoon. tilanne ${pointsHome} - ${pointsOpponent}`);
        } else {
    ...
    return isConnected ? (
        <div className="App">
            <h1 className={"header"}>
                <div style={{width: '80px', marginRight: '10px'}}>{playerServe && <img style={{width: '100%'}} src={"https://upload.wikimedia.org/wikipedia/commons/8/8d/Table_tennis.svg"} />}</div>
                <div>
                    {homeName}
                    <br />
                ...
                <div style={{width: '80px', marginLeft: '10px'}}>{!playerServe && <img style={{width: '100%'}} src={"https://upload.wikimedia.org/wikipedia/commons/8/8d/Table_tennis.svg"} />}</div>
            </h1>
            <br />
            ...
            />
            <br/>
            <p><strong>To start a new game, long press the small silver button (located above the big button) for 2 seconds</strong></p>
        </div>
    ) : (
        <div className="App">
